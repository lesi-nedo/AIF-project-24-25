...
was_hostile(Player, ID) :- character_attack(Player, AttackID), AttackID \= 0, ID is AttackID.
...
player_predict_distance(Player1, Player2, D) :- player_predict_next_position(Player1, FX, FY, FD), player_predict_next_position(Player2, FX2, FY2, FD2), D is (abs(FX1 - FX2) + abs(FY1 - FY2)), Player1 \= Player2.
player_predict_hitbox(Player, L, R, T, B) :- character_box(Player, W, H), player_predict_next_position(Player, FX, FY, FD), L is FX, R is FX + W, T is FY, B is FY + H.
hitbox_will_intesect(Player1, Player2) :- player_predict_hitbox(Player1, L1, R1, T1, B1), player_predict_hitbox(Player2, L2, R2, T2, B2), (R1 > L2, L1 < R2, B1 > T2, T1 < B2).
...
player_is_safe(Player1, Player2, LD) :- over_safe_distance(L), player_predict_distance(Player1, Player2, D), (D >= L), LD is D.
...
should_defend(Player1, Player2) :- hitbox_will_intesect(Player1, Player2), character_hp_energy(Player1, _, Energy), Energy >= 10.
...
optimal_action(Player1, Player2, Action) :- should_defend(Player1, Player2), Action is defend.
optimal_action(Player1, Player2, Action) :- should_use_projectile(Player1, Player2, AttackID),  Action is AttackID.
optimal_action(Player1, Player2, Action) :- should_use_martial(Player1, Player2, AttackID), Action is AttackID.
optimal_action(Player1, Player2, Action) :- should_evade_or_block(Player1, Player2), Action is evade.
optimal_action(Player1, Player2, Action) :- Action is wait.